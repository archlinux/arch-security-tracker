...
def test_delete_user(db, client):
    resp = client.post(url_for('main.delete_user', username=USERNAME), follow_redirects=True,
...
def test_delete_last_admin_fails(db, client):
    resp = client.post(url_for('main.delete_user', username=DEFAULT_USERNAME), follow_redirects=True,
                       data=dict(confirm='confirm'))
...
def test_create_user(db, client):
    ...
    resp = client.post(url_for('main.create_user'), follow_redirects=True,
                       data=dict(username=USERNAME, password=PASSWORD,
...
def test_edit_user(db, client):
    ...
    resp = client.post(url_for('main.edit_user', username=USERNAME), follow_redirects=True,
...
def test_deactive_user(db, client):
    ...
    resp = client.post(url_for('main.login'), data={'username': USERNAME, 'password': PASSWORD})
    assert_not_logged_in(resp, status_code=Unauthorized.code)
